...
其中name参数指定项目的名称，database 指定将要使用的数据库。
...
实体Entity是由 @Entity() 装饰器装饰的一个类,这个类会映射为数据库中的一个表。所以要定义一个实体是很简单的只需要声明一个类并用 @Entity() 装饰器装饰即可。此时声明的实体类就是一个表,但是数据库中表的列还没声明。
...
总之我们在原始sql里需要的在 typeorm  里都有对应的装饰器或者其它的东西一一对应。
...
1. EntityManager 实体管理者:它是一个存放实体存储库集合的地方,通过它可以对任意实体进行管理（insert, update, delete, load 等）使用 EntityManager 你可以操纵应用中的任何实体。我们生成的数据源实例中就有一个对应 EntityManager 的属性 manager。
...
AppDataSource.initialize()
    .then(async () => {
        ...
        // 通过实体管理者操作数据库的crud
        // 查找--
        // find(实体类名)方法 相当于select * from user;查找所有。
        // findOne(实体类名,条件对象)方法 相当于select * from user where id=1;查找第一条记录。
        // findBy(实体类名,条件对象)
        // find(实体类名,条件对象)传入一个配置对象来设置查找条件。
        // 查找所有用户
        ...
        // 多种条件查询
        const user = await AppDataSource.manager.find(User,{
             select: ["firstName", "lastName"],// 指定查询的列名
        ...
        // 这个方法的作用：先查询一次数据库来确定是插入还是修改。如果实体已存在于数据库中，则会修改更新。 如果该实体尚未存在于数据库中，则将其插入新增。 
        // save(obj) ,新增或修改一条
        // save(实体类名, [obj1,obj2,obj3...]) 新增或修改多条
        ...
        // 等等，也可以通过 query 方法使用原生查询、不过复杂查询(涉及到多个表，也就是多个 Entity 的关联查询)一般都是用 query builder 查询器。它允许你使用优雅便捷的语法构建 SQL 查询，执行并获得自动转换的实体。
        const users = await AppDataSource.manager.query('select * from user where age in(?, ?)', [21, 22]);
        console.log(users);


        // 创建一个查询器 
        // createQueryBuilder(实体类名, 别名),也可以不传参。
        const queryBuilder = await AppDataSource.manager.createQueryBuilder();
        const user = await queryBuilder.select("name")
             .from(User, "user")// 指定表和别名。
        ...
        //  两者区别：一个是选项式的一个是函数式，其实本质是一样的。一般涉及到多个表时使用查询器，简单点查询直接 find 指定 where 条件就行。
...
TypeORM 是把表、字段、表和表的关系映射成 Entity 的 class、属性、Entity 之间的关系，那如何映射这种一对一、一对多、多对多的关系呢？还是通过装饰器来指定的。

### 1 一对一关系(OneToOne)
一对一关系,这里还是以用户和身份证表为例。之前是通过在从表中指定的外键来关联，现在通过装饰器来指定这种关系。TypeORM 里一对一关系的映射通过 @OneToOne 装饰器来声明，维持外键列的 Entity 实体添加 @JoinColumn 装饰器。如果是非外键列的 Entity，想要关联查询另一个 Entity，则需要通过第二个参数指定外键列是另一个 Entity 的哪个属性。
...
### 2 一对多关系(OneToMany)
一对多关系,在多的一方通过使用 @ManyToOne() 装饰器,比一对一简单了不再需要 @JoinColumn 因为一对多的关系只可能是在多的那一方保存外键,不过一对多关系更多还是在一的那一方来保持关系,这是需要一个 @OneToMany()  装饰器并同样要通过第二个参数指定外键列在哪张表维护。
...
export class Article {
    ...
    // @JoinTable 指定中间表,可以传入一个配置对象声明中间表名@JoinTable({ name: 'article_tag' })
    // @ManyToMany 声明多对多关系
    @JoinTable()
